...
import { editVisitProcedureSchema } from "../../schemas";
import {  editVisitProcedure } from "../../api";

const EditVisitProcedureForm = () => {
  ...
  const onSubmit = async (values, actions) => {
    try {
      ...
      const data = {visitID: values.visitID, procedureID: values.procedureID, newData: []}
      if(values.cost){
        data.newData.push({column: "cost", value: values.cost})
      }
      
      const res = await editVisitProcedure(data);
  ...
  const formik = useFormik({
    ...
    validationSchema: editVisitProcedureSchema,
    onSubmit: onSubmit,
  });
...
export default EditVisitProcedureForm;
