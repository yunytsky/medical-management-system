...
import { querySchema1 } from "../../schemas";
import {getTotalCostForPatientX } from "../../api";
...
const QueryForm1 = () => {
  ...
  const onSubmit = async (values, actions) => {
    try {
      ...
      const res = await getTotalCostForPatientX(`?patientID=${values.patientID}`);
      setData(res.data.totalCost);
      if(!res.data.totalCost || res.data.totalCost.length <1){
        setNoData(true);
      }
    } catch (error) {
      console.log(error)
